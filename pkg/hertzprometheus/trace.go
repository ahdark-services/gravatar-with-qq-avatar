...
func genLabels(ctx *app.RequestContext) prom.Labels {
	...
	labels[labelMethod] = defaultValIfEmpty(bytestring.BytesToString(ctx.Request.Method()), unknownLabelValue)
	labels[labelStatusCode] = defaultValIfEmpty(strconv.Itoa(ctx.Response.Header.StatusCode()), unknownLabelValue)
	labels[labelPath] = defaultValIfEmpty(bytestring.BytesToString(ctx.Request.Path()), unknownLabelValue)
